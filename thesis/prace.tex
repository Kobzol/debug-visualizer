...
\appendix
\begin{section}{Struktura a instalace}
	% TODO: examples, úprava balíčků
	...
	\parspace Pro kompletní instalaci programu je nutné mít GTK3+, překladač C++ a Pythonovské balíčky \texttt{enum}, \texttt{jsonpickle}, \texttt{matplotlib}
	a \texttt{clang}. Pro spuštění testů je potřeba balíček \texttt{pytest} a pro vygenerování dokumentace balíček \texttt{epydoc}.
	O sestavení programu se stará program waf. Pro jeho použití stačí spustit v interpreteru příkazové řádky ve složce projektu \texttt{./waf <příkaz>}.
	Je možné pro něj použít následující příkazy.
	...
	\begin{description}
		...
		\item[cleanall] vymaže dokumentaci a soubory s byte kódem Pythonu
		%TODO: cleanall > clean?
	\end{description}
	
	%TODO: quick install script?
	\parspace Pro rychlou instalaci je vhodné použít příkazy \texttt{download}, \texttt{configure} a \texttt{build} (v tomto pořadí).
	Pro otestování aplikace lze použít příkaz \texttt{py.test tests}.
